...
final class TLSTransportTests: XCTestCase {
    ...
    func testRestartAfterAFailedRestartReportsSuccessWithoutRecovering() throws {
        ...
        try withPJSUA {
            ...
            guard broken.isError else {
                // Not a skip: if the break-the-listener restart now succeeds, the
                // premise this test exists to observe has changed (restart stopped
                // validating credentials, or validates lazily) — that is a finding,
                // not an unsatisfiable precondition.
                XCTFail("the restart meant to break the listener succeeded; the "
                        + "failed-restart trap is gone and this test needs re-evaluating")
                return
            }
