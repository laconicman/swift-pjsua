...
| [acc-modify-disable-reg-still-destroys-regc](acc-modify-disable-reg-still-destroys-regc.md) | `disable_reg_on_modify` promises "apply this config without touching my registration", but the regc is still destroyed — silently stopping registration refresh | precedence rules in the app (`offhook/docs/Push-vs-Active-Socket.md` §1.2) | **verified 2026-08-04 vs local master `4896a5e6a`**, sources read directly — documentation issue (draft offers a behaviour reading too) |
