...
- Relates: TD-19 (a TLS listener restart drops its credentials — but **not** via this path:
  `pjsua_handle_ip_change()`'s `restart_listener()` passes a NULL `opt`, preserving credentials
  by construction; the hazard is only our own `pjsua_transport_lis_restart()`), TD-22 (439),
  `Push-vs-Active-Socket.md` §5.
...
- Note: [`pjproject-5154`](../Upstream/pjproject-5154-439-first-hop-lacks-outbound.md). The
  ...
  on the fork as [laconicman#15](https://github.com/laconicman/pjproject/pull/15).

## TD-29 — TLS now fails fast at start, and our only recovery path is TD-19 · open (blocks TLS) · **measured 2026-09-01**
